...
export default function Sales() {
  ...
  const [detail, setDetail] = useState<SaleDetails | null>(null);
  const [form, setForm] = useState({ clientName: '', discount: '', notes: '' });
  const [items, setItems] = useState<ItemForm[]>([{ productId: '', quantity: '1', unitPrice: '' }]);

  const load = () => {
    setLoading(true);
    request('/sales?pageNumber=1&pageSize=50')
      .then((d: any) => setSales(d.data?.data ?? []))
  ...
  const openCreate = async () => {
    setFormError(null);
    setForm({ clientName: '', discount: '', notes: '' });
    setItems([{ productId: '', quantity: '1', unitPrice: '' }]);
    setModalOpen(true);
    try {
      const d: any = await request('/products?pageNumber=1&pageSize=200&isActive=true');
  ...
  const onProductChange = (index: number, productId: string) => {
    ...
    setItem(index, { productId, unitPrice: product?.salePrice?.toString() || '' });
  };
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      setFormError(err?.message || 'Falha ao criar venda');
    } finally {
      setSaving(false);
    }
  };

  const doAction = async (sale: Sale, action: 'confirm' | 'cancel') => {
    if (action === 'cancel' && !window.confirm(`Cancelar a venda ${sale.number}?`)) return;
    try {
      const body = action === 'cancel' ? JSON.stringify({ reason: 'Cancelado pelo usuário' }) : undefined;
      await api(`/sales/${sale.id}/${action}`, { method: 'POST', ...(body ? { body, headers: { 'Content-Type': 'application/json' } } : {}) });
      load();
    } catch (err: any) {
  ...
  const openDetail = async (id: string) => {
    try {
      const d = await api<SaleDetails>(`/sales/${id}`);
      setDetail(d);
    } catch (err: any) {
      setError(err?.message);
    }
  ...
  return (
    <div>
      <PageHeader
        title="Vendas"
        subtitle="Pedidos e orçamentos de venda"
        action={<button className={btnPrimary} onClick={openCreate}>+ Nova venda</button>}
      />
              ...
              {sales.map((s) => (
                <tr key={s.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                  <td className="px-5 py-3 font-medium text-slate-700">
                    <button className="hover:text-indigo-600 hover:underline" onClick={() => openDetail(s.id)}>{s.number}</button>
                  </td>
                  <td className="px-5 py-3 text-slate-500">{formatDateTime(s.saleDate)}</td>
                  <td className="px-5 py-3 text-slate-700">{s.clientName || '—'}</td>
                  <td className="px-5 py-3">
                    <Badge tone={statusTone(s.status, 7)}>{SALE_STATUS[s.status] ?? s.status}</Badge>
                  </td>
                  <td className="px-5 py-3 text-right font-medium text-slate-800">{formatCurrency(s.total)}</td>
                  <td className="px-5 py-3">
                    <div className="flex justify-end gap-2">
                      {s.status === 0 && (
                        <>
                          <button className={btnSuccess} onClick={() => doAction(s, 'confirm')}>Confirmar</button>
                          <button className={btnDanger} onClick={() => doAction(s, 'cancel')}>Cancelar</button>
                        </>
                      )}
                      {s.status === 1 && (
                        <button className={btnDanger} onClick={() => doAction(s, 'cancel')}>Cancelar</button>
            ...
            <div className="space-y-2">
              {items.map((item, index) => (
                <div key={index} className="grid grid-cols-[1fr_90px_110px_32px] items-center gap-2">
                  ...
                  />
                  <input
                    type="number" min="0" step="0.01" className={inputClass} placeholder="Preço"
      ...
      <Modal open={!!detail} title={`Venda ${detail?.number ?? ''}`} onClose={() => setDetail(null)}>
        {detail && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Badge tone={statusTone(detail.status, 7)}>{SALE_STATUS[detail.status] ?? detail.status}</Badge>
              <span className="text-xs text-slate-500">{formatDateTime(detail.saleDate)}</span>
            </div>
            <div className="text-sm text-slate-600">Cliente: {detail.clientName || '—'}</div>
            <table className="w-full text-sm">
              <thead>
